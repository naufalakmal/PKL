...
class ref_kecamatan_Model extends CI_Model
{
	...
	function getAll($filter = null, $limit = 20, $offset = 0, $orderBy, $orderType)
	{
		...
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS pg.*,p.nama_kota,q.nama_provinsi
								   FROM " . $this->table . " pg
								   LEFT JOIN ref_kota p on p.id_kota = pg.kota_id
									 LEFT JOIN ref_provinsi q on p.provinsi_id = q.id_provinsi
								   $where group by pg.id_kecamatan ORDER BY $orderBy $orderType $limitOffset
								   ");
