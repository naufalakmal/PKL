<?php
class master_tujuan_Model extends CI_Model
{
		...
		if (isset($filter)) {
			if (!empty($filter->keyword)) {
				$cond[] = "(lower(" . $this->key . ") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(nama_tujuan) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(telepon_tujuan) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								)";
			}
	...
	function getAllViapool($filter = null, $limit = 20, $offset = 0, $orderBy, $orderType)
	{
		$where = "";
		$cond = array();
		if (isset($filter)) {
		...
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS *
								   FROM master_tujuan pg
									 LEFT JOIN ref_kecamatan g on g.id_kecamatan = pg.id_kecamatan
									 LEFT JOIN ref_kota p on p.id_kota = pg.id_kota
									 LEFT JOIN ref_provinsi q on p.provinsi_id = q.id_provinsi where jenis_tujuan='Pool'
								   $where ORDER BY $orderBy $orderType $limitOffset
								   ");
	...
	{
		$where = "WHERE " . $this->key . " <> '" . $this->db->escape_str(strtolower($id)) . "' and nama_tujuan = '" . $this->db->escape_str(strtolower($name)) . "' ";
		$query = $this->db->query("SELECT  *
								   FROM " . $this->table . "
								   $where
								   ");
	...
	{
		$query = $this->db->query(" ( SELECT  " . $this->key . " FROM transaksi_amplop where " . $this->key . " = '" . $this->db->escape_str(strtolower($id)) . "' ) ");
		$result = $query->row();
		$query->free_result();
	...
	{
		$query = $this->db->get('master_tujuan');
		if ($query->num_rows() > 0) {
