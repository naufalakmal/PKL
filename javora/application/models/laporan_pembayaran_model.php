...
class Laporan_pembayaran_Model extends CI_Model
{
	...
	function __construct()
    {
		...
		function getAllLaporanPiutangPerPelanggan($filter = null,$limit = 20,$offset = 0, $orderBy, $orderType)
		{
		  	...
		  	if (isset($filter))
		  	{
				...
				if (!empty($filter->status))
				{
					if(strtolower($filter->status) != "all")
						...
						$cond[] = "(lower(p.nama) like '%"  . $this->db->escape_str(strtolower($filter->status)) . "%')";
				}

				if (!empty($filter->from) || !empty($filter->to))
				{
					$cond[] = "(pg.tanggal_bayar_amplop >= '" . $this->db->escape_str($filter->from) . "' and pg.tanggal_bayar_amplop <= '" . $this->db->escape_str($filter->to) . "' )";
				}
			...
			$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS pg.*,p.*, sum(pg.terbayar) as totalkredit

									   FROM transaksi_pembayaran_amplop pg
	...
	public function get_nama_pelanggan($field, $value = "",$obj = false)
	{
		if(!$field)
			$field = 'id_pelanggan';

		$where = "WHERE $field = '".$this->db->escape_str(strtolower($value))."'";
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS pg.nama
	...
	function getAllLaporanPiutangDetailPerPelanggan($id)
	{
		...
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS pg.*,p.nama
									 FROM transaksi_pembayaran_amplop pg
									 LEFT JOIN master_pelanggan p on p.id_pelanggan = pg.id_pelanggan
