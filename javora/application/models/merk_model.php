...
class Merk_model extends CI_Model
{
		...
		if (isset($filter)) {
			if (!empty($filter->keyword)) {
				$cond[] = "(lower(" . $this->key . ") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(nama_merk) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
	...
	{
		$where = "WHERE " . $this->key . " <> '" . $this->db->escape_str(strtolower($id)) . "' and nama_merk = '" . $this->db->escape_str(strtolower($name)) . "' ";
		$query = $this->db->query("SELECT  *
								   FROM " . $this->table . " 
								   $where 
								   ");
	...
	{
		$query = $this->db->query(" ( SELECT  " . $this->key . " FROM mobil where " . $this->key . " = '" . $this->db->escape_str(strtolower($id)) . "' ) ");
		$result = $query->row();
		$query->free_result();
