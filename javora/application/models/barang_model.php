...
class Barang_Model extends CI_Model
{
	...
	function getAll($filter = null, $limit = 20, $offset = 0, $orderBy, $orderType)
	{
		...
		if (isset($filter)) {
			if (!empty($filter->keyword)) {
				$cond[] = "(lower(" . $this->key . ") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 ...
								 or lower(b.qty) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
		...
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS b.*
								   FROM " . $this->table . " b
	...
	public function get_by($field, $value = "", $obj = false)
	{
		...
		$query = $this->db->query("SELECT b.*,k.nama_kategori kategori_barang,k.keterangan
								   FROM " . $this->table . " b
								   LEFT JOIN kategori_barang k on k.id_kategori = b.id_kategori
								   $where
								   ");
	...
	{
		$where = "WHERE " . $this->key . " <> '" . $this->db->escape_str(strtolower($id)) . "' and nama_barang = '" . $this->db->escape_str(strtolower($name)) . "' ";
