...
class Transaksi_order extends Admin_Controller {
	...
	public function save()
	{
		...
		if($post)
		{
...

			{
				...
				$this->transaksi_order_model->save_detail($data_history);
				...
				if(!empty($id))
				{
					$this->transaksi_order_model->remove_detail($id);
				}
				...
				$this->session->set_flashdata('admin_save_success', "data berhasile disimpan");
					...
					$message = "Status Barang Diterima Oleh STB";
	...
	public function validasi($id = "")
	{
		...
		if(!empty($id))
		{
			...
			else
			{
				...
				$data_piutang['keterangan'] = $data['keterangan'];
	...
	public function generate_code()
	{
		...
		if (!empty($last)) {
			$number = substr($last->id_amplop, 9, 8) + 1;
			//var_dump($number);
			//exit();
