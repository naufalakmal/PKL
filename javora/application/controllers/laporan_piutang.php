...
class Laporan_piutang extends Admin_Controller {
	...
	public function index()
	{
		...
		$data['layout'] = "laporan_piutang/index";
		...
		list($data['data'],$total) = $this->laporan_piutang_model->getAll($filter,$limit,$offset,$orderBy,$orderType);

		$this->load->library('pagination');
		$config['base_url'] = site_url("laporan_piutang?");
	...
	public function manage($id = "")
	{
		...
		$data['layout'] = "laporan_piutang/manage";
		...
		if($id)
		{
			$dt =  $this->laporan_piutang_model->get_by("pg.no_bayar_amplop",$id,true);
			if(!empty($dt))
				$data['data'] = $dt;
		}
	...
	public function save()
	{
		...
		if($post)
		{
			...
			if(empty($error))
			{
				if(empty($id))
				{
					$ceklaporan_piutang = $this->laporan_piutang_model->get_by("pg.no_bayar_amplop",$post['no_bayar_amplop']);
					if(!empty($ceklaporan_piutang))
			...
			if(empty($error))
			{
				...
				$save = $this->laporan_piutang_model->save($id,$data,false);
				$ubah_piutang = $this->laporan_piutang_model->ubah_piutang($id_amplop,$data_ubah_daftar_piutang,false);
				$datailkode = $post['detail']['id_barang'];
				$datailjumlah = $post['detail']['qty'];

				if(!empty($id))
				{
					$this->laporan_piutang_model->remove_detail($id);
				}
				foreach($datailkode as $key => $val)
				{
					...
					$this->laporan_piutang_model->save_detail($detail);
				}
				...
				if($post['action'] == "save")
					redirect("laporan_piutang/manage/".$id);
				else
					redirect("laporan_piutang");
			}
			else
			{
				$err_string = "<ul>";
	...
	public function delete($id = "")
	{
		...
		if(!empty($id))
		{
			$cek = $this->laporan_piutang_model->get_by("pg.no_bayar_amplop",$id,true);
			if(empty($cek))
			{
			...
			{
				$this->laporan_piutang_model->remove($id);
	...
	public function generate_code()
	{
		...
		$last = $this->laporan_piutang_model->get_last();
		if (!empty($last)) {
	...
	public  function cetak($id)
	{
		...
		$data['layout'] = "laporan_piutang/cetak";
	...
	public function rekap()
	{
		...
		$data['layout'] = "laporan_piutang/rekap";
		...
		list($data['data'],$total) = $this->laporan_piutang_model->getAllLaporanPiutangPerPelanggan($filter,0,0,"pg.id_pelanggan","desc");
	...
	public function rekaplanjut($id = "")
	{
		...
		$data['title'] = "Laporan Piutang Pelanggan";
		$data['layout'] = "laporan_piutang/rekaplanjut";
		$action = $this->input->get('actiondetail');
		$cek_nama = $this->laporan_piutang_model->get_nama_pelanggan("pg.id_pelanggan",$id,true);

		$data['title1'] = $cek_nama->nama;
		$data['id'] = $id;
		list($data['data'],$total) = $this->laporan_piutang_model->getAllLaporanPiutangDetailPerPelanggan($id,0,0,"pg.id_pelanggan","desc");
		if($action)
		{
	...
	public function export($action,$data,$filter)
	{
		...
		if(empty($data))
		{
			...
			redirect("laporan_piutang/rekap?from=".$filter->from."&to=".$filter->to."&status=".$filter->status."");
		}
		else
		{
			if($action == "excel")
	...
	public function exportdetail($action,$data,$id)
	{
		...
		//var_dump($data);
		//exit();
		$title = "Rek. Koran Piutang an $nama";
		$file_name = $title."_".date("Y-m-d");
		$headerTitle = $title;
		...
		{
			$this->session->set_flashdata('admin_save_error', "data tidak tersedia");
			redirect("laporan_piutang/rekaplanjut");
		}
		else
		{
	...
	public function generate_format_detail($data)
	{
...

		{
			...
			$dat['Deskripsi'] = $dt['deskripsi'];
			$dat['No Bukti'] = $dt['no_bukti'];
			$dat['Debit'] = $dt['debit'];
			$dat['Kredit'] = $dt['kredit'];
			$dat['Saldo'] = $dt['saldo'];
			$newdata[] = $dat;
		}
	...
	public function generate_format($data)
	{
		...
		foreach($data as $key => $dt)
		{
			...
			$dat['Debit'] = $dt['jumlah_hutang'];
			$dat['Kredit'] = $dt['jumlah_sudah_bayar'];
			$dat['Saldo'] = $dt['jumlah_sisa_piutang'];
