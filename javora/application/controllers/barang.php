...
class Barang extends Admin_Controller
{
	...
	public function manage($id = "")
	{
		...
		list($data['kategori_barang'], $total) = $this->kategori_model->getAll(null, null, null, null, null);
	...
	public function save()
	{
		...
		if ($post) {
			...
			if (!empty($post['berat_barang']))
				$data['berat_barang'] = $post['berat_barang'];
			else
			...
			if (empty($error)) {
				if (empty($id)) {
					...
					$cek = $this->barang_model->get_by("b.nama_barang", $post['nama_barang']);
					if (!empty($cek))
						$error[] = "nama sudah terdaftar";
				} else {
					$cek = $this->barang_model->cekName($id, $post['nama_barang']);
					if (!empty($cek))
						$error[] = "nama sudah terdaftar";
				}
			}
