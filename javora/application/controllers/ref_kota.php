...
class ref_kota extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model("ref_kota_model");
	...
	public function index()
	{
		...
		$data['title'] = "DATA REFERENSI KOTA";
		$data['layout'] = "ref_kota/index";
		...
		list($data['data'], $total) = $this->ref_kota_model->getAll($filter, $limit, $offset, $orderBy, $orderType);

		$this->load->library('pagination');
		$config['base_url'] = site_url("ref_kota?");
	...
	public function manage($id = "")
	{
		...
		if ($id) {
			$dt =  $this->ref_kota_model->get_by("pg.id_kota", $id, true);
			if (!empty($dt))
				$data['data'] = $dt;
		}
	...
	public function save()
	{
		...
		if ($post) {
			...
			if (empty($error)) {
				if (empty($id)) {
					$cekref_kota = $this->ref_kota_model->get_by("pg.id_kota", $post['id']);
					if (!empty($cekref_kota))
			...
			if (empty($error)) {
				//var_dump($data);
				//exit();
				$save = $this->ref_kota_model->save($id, $data, false);


				$this->session->set_flashdata('admin_save_success', "data berhasil disimpan");

				if ($post['action'] == "save")
					redirect("ref_kota/manage/" . $id);
				else
					redirect("ref_kota");
			} else {
				$err_string = "<ul>";
	...
	public function delete($id = "")
	{
		...
		if (!empty($id)) {
			$cek = $this->ref_kota_model->get_by("pg.id_kota", $id, true);
			if (empty($cek)) {
				$this->session->set_flashdata('admin_save_error', "ID tidak terdaftar");
				redirect("ref_kota");
			} else {
				$this->ref_kota_model->remove($id);
